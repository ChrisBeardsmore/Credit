...
# --- Page Setup ---
st.set_page_config(page_title="Dyce Contract Decision Engine V24", layout="wide")
...
st.title(f"\u26a1 Dyce Contract Decision Engine V24 ({VERSION})")
...
config = load_config()
limits_df = config['ApprovalMatrix']
sic_df = load_sic()

# --- Inputs ---
st.header("1\ufe0f\ufe0f Business Details")
...
def get_required_approver(sites, spend, volume):
    for _, row in limits_df.iterrows():
        try:
            if (
                int(sites) <= int(row['Max Sites']) and
                float(spend) <= float(row['Max Spend (£)']) and
                float(volume) <= float(row['Max Volume (kWh)'])
            ):
                return row['Role']
