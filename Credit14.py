...
smet_compatible = st.radio("Is Meter SMET-Compatible?", ["Yes", "No"], horizontal=True)
has_ccjs = st.radio("Any CCJs or Defaults?", ["Yes", "No"], horizontal=True)
payment_terms = st.selectbox("Requested Payment Terms", ["7 Days Direct Debit", "14 Days BACS", "28 Days BACS"], index=0)
...
def export_pdf(data, decision, approver, reasons):
    ...
    buffer = BytesIO()
    pdf.output(buffer, dest='F').close()
    buffer.seek(0)
    return buffer
