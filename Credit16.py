...
CONFIG_URL = "https://raw.githubusercontent.com/ChrisBeardsmore/Credit/main/Config/Credit_Decision_Config_Template.xlsx"
...
st.markdown("""
    <style>
        ...
        label, .stMarkdown, .css-1cpxqw2, .stRadio label, .st-bf {
...
def get_required_approver(sites, spend, volume):
    for _, row in limits_df.iterrows():
        if (sites <= row['Max Sites'] and spend <= row['Max Spend'] and volume <= row['Max Volume']):
            return row['Role']
