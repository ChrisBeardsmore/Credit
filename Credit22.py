...
# --- Page Setup ---
st.set_page_config(page_title="Dyce Contract Decision Engine V22", layout="wide")
...
st.markdown("""
    <style>
        ...
        .stRadio > div { color: rgb(15,42,52) !important; }
    </style>
""", unsafe_allow_html=True)

st.image(LOGO_PATH, width=200)
st.title(f"⚡ Dyce Contract Decision Engine V22 ({VERSION})")
...
def get_required_approver(sites, spend, volume):
    for _, row in limits_df.iterrows():
        if (
            sites <= row['Max Sites'] and
            spend <= row['Max Spend (£)'] and
            volume <= row['Max Volume (kWh)']
        ):
            return row['Role']
